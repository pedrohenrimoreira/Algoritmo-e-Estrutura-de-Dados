...
void inserir(SWCharacter *self)
{
    Celula *tmp = novaCelula(self);
    tmp->prox = topo;
    topo = tmp;
    tmp = NULL;
    total++;
}
void imprimir(Celula *tmp, int pos) {
    if(tmp->prox != NULL) {
        imprimir(tmp->prox, pos-1);
    }
    printf("[%d] ", pos);
...
int main()
{
    ...
    for (int i = 0; i < mov; i++)
    {
        ...
        if (str[0] == 'I')
        {
            inserir(SWCharacter_from_file(&str[2]));
        }
        else if (str[0] == 'R')
        {
            SWCharacter *removido = remover();
            printf("(R) %s\n", removido->name);
        }
    }
    imprimir(topo, total-1);
    // swc_free(array, len);
    // free(str);
    return 0;
}